  ...
  return (
    <main className="min-h-screen bg-[var(--background)] text-[var(--foreground)]">
      <div className="mx-auto max-w-[1400px] px-5 py-6 sm:px-8 lg:px-10">
        <header className="marketing-nav sticky top-4 z-20 rounded-full px-5 py-3 animate-fade-up">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex items-center justify-between gap-4">
              <Link className="display-title text-lg font-semibold tracking-[-0.04em]" href="/">
                Bugslinter
              </Link>
              <div className="meta-mono hidden items-center gap-2 rounded-full border border-white/10 bg-white/[0.03] px-3 py-1 text-[11px] uppercase tracking-[0.24em] text-[var(--muted)] sm:flex">
        ...
        <section className="relative overflow-hidden px-1 pb-14 pt-12 sm:pt-16 animate-fade-up">
          ...
          <div className="grid gap-10 lg:grid-cols-[0.86fr_1.14fr] lg:items-end">
            <div className="relative z-10 max-w-2xl animate-stagger">
              ...
              <h1 className="display-title mt-5 text-5xl font-semibold leading-[0.92] tracking-[-0.07em] text-[var(--foreground)] sm:text-6xl xl:text-7xl">
                Delivery context stays intact from incoming bug to approved release.
              </h1>
              <p className="editorial-copy mt-6 max-w-xl text-lg text-[var(--foreground-subtle)]">
              ...
              <div className="line-list mt-10 grid max-w-xl gap-0 rounded-[1.6rem] border border-white/10 bg-white/[0.02] animate-stagger">
              ...
              <div className="hero-frame animate-fade-up">
                ...
                <div className="hero-shell">
                  <div className="hero-header animate-stagger">
                    <div>
                      <p className="meta-mono text-[11px] uppercase tracking-[0.26em] text-[var(--muted)]">Future-facing workflow view</p>
                      <h2 className="display-title mt-3 text-2xl font-semibold tracking-[-0.04em]">
                  ...
                  <div className="hero-grid animate-stagger">
                    ...
                    <div className="hero-bridge">
                      <div className="hero-ai-core animate-fade-up">
                        <p className="meta-mono text-[11px] uppercase tracking-[0.24em] text-cyan-200/80">AI workflow core</p>
                        <h3 className="display-title mt-3 text-xl font-semibold tracking-[-0.04em]">
        ...
        <section id="workflow" className="section-frame py-14 animate-fade-up">
          <div className="grid gap-8 lg:grid-cols-[0.78fr_1.22fr]">
            <div className="max-w-xl">
              ...
              <p className="editorial-copy mt-5 text-base text-[var(--foreground-subtle)]">
                Bugslinter is built for teams that already know the pain: bug context scattered
                across trackers, chat, pull requests, and release spreadsheets. The page should
                sell a delivery system that brings that loop back under control.
              </p>
            </div>

            <div className="panel-strong rounded-[2rem] p-5 sm:p-6">
              <div className="line-list grid gap-0 animate-stagger">
                {workflowSteps.map((step) => (
                  <div key={step.title} className="grid gap-3 px-1 py-5 md:grid-cols-[130px_minmax(0,1fr)]">
                    <p className="meta-mono text-[11px] uppercase tracking-[0.26em] text-[var(--muted)]">{step.eyebrow}</p>
                    <div>
                      <h3 className="display-title text-2xl font-semibold tracking-[-0.04em]">{step.title}</h3>
                      <p className="mt-3 max-w-2xl text-sm leading-7 text-[var(--foreground-subtle)]">{step.body}</p>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <section id="integrations" className="section-frame py-14 animate-fade-up">
          <div className="panel rounded-[2rem] px-6 py-7 sm:px-8">
            <div className="grid gap-8 lg:grid-cols-[0.72fr_1.28fr]">
              <div>
                ...
                <p className="editorial-copy mt-5 max-w-lg text-base text-[var(--foreground-subtle)]">
              ...
              <div className="grid gap-4 md:grid-cols-3 animate-stagger">
                {productPillars.map((pillar) => (
                  <div key={pillar.title} className="rounded-[1.8rem] border border-white/10 bg-white/[0.03] px-5 py-6">
                    <p className="meta-mono text-[11px] uppercase tracking-[0.22em] text-[var(--muted)]">Integration lane</p>
                    <h3 className="display-title mt-4 text-lg font-semibold tracking-[-0.03em]">{pillar.title}</h3>
        ...
        <section id="contrast" className="section-frame py-14">
          <div className="grid gap-6 lg:grid-cols-[1.08fr_0.92fr]">
            <div className="panel rounded-[2rem] px-6 py-7 sm:px-8">
              ...
              <div className="line-list mt-7 grid gap-0">
                {comparisonPoints.map((point) => (
                  <div
                    key={point}
                    className="px-1 py-4 text-sm text-[var(--foreground-subtle)]"
                  >
                    {point}
                  </div>
                ))}
            ...
            <div className="panel-strong rounded-[2rem] px-6 py-7 sm:px-8">
              <p className="section-kicker">Current proof</p>
              <h2 className="display-title mt-4 text-2xl font-semibold tracking-[-0.04em]">
                Built from a working product surface, presented as the sharper version buyers need to see.
              </h2>
              <div className="mt-6 rounded-[1.7rem] border border-white/10 bg-[linear-gradient(180deg,rgba(17,25,43,0.92),rgba(11,17,32,0.96))] p-5">
                <div className="flex items-center justify-between gap-4">
                  <div>
                    <p className="meta-mono text-[11px] uppercase tracking-[0.24em] text-[var(--muted)]">Current workspace base</p>
        ...
        <section className="section-frame py-14">
          <div className="panel-strong rounded-[2.2rem] px-6 py-8 sm:px-8 lg:px-10">
...
function HeroSignal({ label }: { label: string }) {
  return (
    <div className="flex items-center justify-between gap-4 px-4 py-4">
      ...
      <span className="meta-mono text-xs uppercase tracking-[0.2em] text-[var(--muted)]">Verified</span>
    </div>
  );
}
...
function WorkflowNode({
  ...
  return (
    <div
      className={`workflow-node ${connector ? "workflow-connector" : ""} ${
        tone === "accent"
...
function MiniPanel({ title, value }: { title: string; value: string }) {
  return (
    <div className="rounded-[1.25rem] border border-white/10 bg-white/[0.03] px-4 py-4">
      <p className="meta-mono text-[11px] uppercase tracking-[0.22em] text-[var(--muted)]">{title}</p>
